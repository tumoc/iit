                ...
                if (ModelState.IsValid)
                {
                    ...
                    try
                    {
                        var room = new Room
                        {
                            ...
                            Price = model.Price,
                            PricePremium = model.PricePremium,
                        ...
                        // them tien ich cao cap
                        if (model.EPremiumItems != null)
                        {
                            foreach (EPremiumItem item in model.EPremiumItems)
                            {
                                var roomPremiumFacility = new RoomFacility
                                {
                                    FacilityID = item.Id,
                                    RoomID = room.ID,
                                };
                                db.RoomFacilities.InsertOnSubmit(roomPremiumFacility);
                            }
                            db.SubmitChanges();
                        }
        ...
        public ActionResult Update(int id)
        {
            using (var db = new MyDbDataContext())
            {
                ...
                var room = new ERoom
                {
                    ...
                    PricePremium = detailRoom.PricePremium,
                ...
                room.EPremiumItems = db.RoomFacilities.Where(a => a.RoomID == detailRoom.ID).Select(a => new EPremiumItem
                {
                    Id = a.ID
                }).ToList();
                return View(room);
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        if (room != null)
                        {
                            ...
                            room.PricePremium = model.PricePremium;
                            ...
                            //Thêm tien ich cao cap cho phòng
                            if (model.EPremiumItems != null)
                            {
                                foreach (EPremiumItem item in model.EPremiumItems)
                                {
