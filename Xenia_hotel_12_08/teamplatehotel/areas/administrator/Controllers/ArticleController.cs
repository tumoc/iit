    ...
    public class ArticleController : BaseController
    {
            ...
            using (var db = new MyDbDataContext())
            {
                if (ModelState.IsValid)
                {
                    ...
                    try
                    {
                        ...
                        //Thêm icon cho bài viết
                        if (model.EAmenityArticleItems != null)
                        {
                            foreach (EAmenityArticleItem itemAmentity in model.EAmenityArticleItems)
                            {
        ...
        public ActionResult Update(int id)
        {
            ...
            using (var db = new MyDbDataContext())
            {
                ...
                // ds tiện tích
                artile.EAmenityArticleItems = db.ArticleAmenities.Where(a => a.ArticleID == detailArticle.ID).Select(a => new EAmenityArticleItem
                {
                    Id = a.ID
                }).ToList();
                LoadData();
            ...
            menus =
                MenuController.GetListMenu(0, Request.Cookies["lang_client"].Value).Where(
                    a =>
                        ...
                        a.Type == SystemMenuType.Spa ||
                        a.Type == SystemMenuType.Activities ||
                        a.Type == SystemMenuType.Restaurant ||
                        a.Type == SystemMenuType.Home ||
                        a.Type == SystemMenuType.Gym ||
