...
namespace TeamplateHotel.Controllers
{
    public class HomeController : BasicController
    {
        ...
        public ActionResult Index(object aliasMenuSub, object idSub, object aliasSub, Search search , int? page, int? pageSize)
        {
            ...
            switch (menu.Type)
            {
                ...
                case SystemMenuType.Service:
                    goto Service;
                case SystemMenuType.Activity:
                    goto Tranghoatdong;
                case SystemMenuType.Facility:
        ...
        Tranghoatdong:
            ...
            //Danh sách bài viết
            List<Article> articles2 = CommentController.GetActivityArticles(menu.ID);
            if (articles2.Count == 1)
            {
                DetailArticle detailArticle = CommentController.Detail_Article(articles2[0].ID);
                Article article = db.Articles.FirstOrDefault(b => b.ID == articles2[0].ID);
                CommentController.Plus(article.ID);
                ViewBag.MetaTitle = detailArticle.Article.MetaTitle;
                ViewBag.MetaDesctiption = detailArticle.Article.MetaDescription;
                ViewBag.OGImage = detailArticle.Article.Image;
            ...
            IPagedList<Article> list2 = articles2.ToPagedList(pagenumber2, pagesize2);
            return View("Activity", list2);
        ...
        TrangRoom:
            ...
            int pagenumber3 = page ?? 1;
            int pagesize3 = pageSize ?? 6;
            IPagedList<Service> list3 = services.ToPagedList(pagenumber3, pagesize3);
            return View("Service/ListService", list3);
            #endregion
        }
    }
}